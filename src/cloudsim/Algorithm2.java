...
public class Algorithm2 {
    ...
    private static List<Cloudlet> createCloudlet(int userId, int cloudlets, int idShift) {
        ...
        for (int i = 0; i < cloudlets; i++) {
            ...
            int dcId = rand.nextInt(datacenter_num - 1);
    ...
    public static void StartAlgo2() {
        Log.printLine("Starting Job Scheduler...");
        divideJobs();
        GenerateMatrices();
        try {
            int num_user = 1;   // number of grid users
            Calendar calendar = Calendar.getInstance();//current dates
            boolean trace_flag = false;  // mean trace events

            CloudSim.init(num_user, calendar, trace_flag);

            datacenter = new Datacenter[datacenter_num];//create data center and we need atleast one to run simulation
            for (int i = 0; i < datacenter_num; i++) {
                datacenter[i] = createDatacenter("Datacenter_" + i);
            }
            DatacenterBroker broker = createBroker("Broker_0");//create broker
    ...
    public static Datacenter createDatacenter(String name) {
        ...
        List<Host> hostList = new ArrayList<Host>();//create a list to host machine
        List<Pe> peList = new ArrayList<Pe>();//machine can have one or more cpus or PE
        int mips = 1000;
        peList.add(new Pe(0, new PeProvisionerSimple(mips)));//create pe and add them to list of machines
        int hostId = 0;
        ...
        hostList.add(
          ...
          //create a data center characteristics that stores the properties of data center
          //architecture, os , list of machines, allocation policy , time zone, and its prize
        ...
        Datacenter datacenter = null;//create power data center object
        try {
