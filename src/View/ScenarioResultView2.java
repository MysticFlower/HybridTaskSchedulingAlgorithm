 ...
 * @author shabeena
 */
public class ScenarioResultView2 {
   ...
   // private Label result_label_3 = new Label("Failed Task: ");
    ...
    public TableView<Scenario2> scenario_view;

    public ObservableList<Scenario2> scenario_list;
    ...
    public void initView() {
        ...
        nameBox.getChildren().addAll(result_label, result_view, result_label_2, result_view_2);
        ...
        GroupBox customer_group = new GroupBox("Scenario 2", customerBox, Resource.RIGHT_AREA_WIDTH, 30);
    ...
    public void initResultTable() {
        ...
        scenario_view = new TableView<Scenario2>();
        ...
        TableColumn<Scenario2, String> task_id = new TableColumn<>("Task Id");
        TableColumn<Scenario2, String> datacenter_id = new TableColumn<>("Datacenter ID");
        TableColumn<Scenario2, String> vm_id = new TableColumn<>("Vm Id");
        TableColumn<Scenario2, String> start_time = new TableColumn<>("Start Time");
        TableColumn<Scenario2, String> finish_time = new TableColumn<>("Finish Time");
        TableColumn<Scenario2, String> estimated_time = new TableColumn<>("Estimated Time");
        //TableColumn<Scenario2, String> wait_time = new TableColumn<>("Wait");
        TableColumn<Scenario2, String> status = new TableColumn<>("Status");

        task_id.setCellValueFactory(new PropertyValueFactory<Scenario2, String>("TaskId"));
        datacenter_id.setCellValueFactory(new PropertyValueFactory<Scenario2, String>("DataCenterId"));
        vm_id.setCellValueFactory(new PropertyValueFactory<Scenario2, String>("VmID"));
        start_time.setCellValueFactory(new PropertyValueFactory<Scenario2, String>("StartTime"));
        finish_time.setCellValueFactory(new PropertyValueFactory<Scenario2, String>("EndTime"));
        estimated_time.setCellValueFactory(new PropertyValueFactory<Scenario2, String>("Estimate"));
        //wait_time.setCellValueFactory(new PropertyValueFactory<Scenario2, String>("Wait"));
        status.setCellValueFactory(new PropertyValueFactory<Scenario2, String>("Status"));
        ...
        estimated_time.prefWidthProperty().bind(scenario_view.widthProperty().multiply(0.20));
        //wait_time.prefWidthProperty().bind(scenario_view.widthProperty().multiply(0.14));
        status.prefWidthProperty().bind(scenario_view.widthProperty().multiply(0.30));
    ...
    public void loadContent() {
        scenario_list.clear();
        Algorithm2.datacenter_num = SettingView2.datacenter_numbers;
        Algorithm2.jobs_num = SettingView2.task_numbers;
        //Algorithm2.wait_limit = SettingView2.wait_limit;
        Algorithm2.StartAlgo2();
        Algorithm2.calcMaxEstimatedTime();
        Algorithm2.calcMinEstimatedTime();
        
        int size = Algorithm2.resultList.size();
        Cloudlet cloudlet;

        DecimalFormat dft = new DecimalFormat("###.##");
        //int fail_number = 0;
        for (int i = 0; i < size; i++) {
            cloudlet = Algorithm2.resultList.get(i);
            if (cloudlet.getCloudletStatus() == Cloudlet.SUCCESS) {
                //double waited_time = cloudlet.getWaitingTime();
                //if (waited_time <= Algorithm2.wait_limit){
                scenario_list.add(new Scenario2("" + cloudlet.getCloudletId(), "" + cloudlet.getResourceId(), "" + cloudlet.getVmId(),
        ...
        result_view.setText("" + dft.format(Algorithm2.max_time));
        result_view_2.setText("" + dft.format(Algorithm2.min_time));
